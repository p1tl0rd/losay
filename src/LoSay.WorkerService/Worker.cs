...
namespace LoSay.WorkerService
{
	public class Worker : BackgroundService
	{
		...
		public Worker(ILogger logger, SamplingSettings settings, IServiceProvider serviceProvider,
			IHubContext<PLCHubService> hubContext, SignalRConnectURLSettings signalRConnectURLSettings)
		{
			...
			if (int.TryParse(settings.samp, out int interval))
			{
				_timer = new PeriodicTimer(TimeSpan.FromSeconds(interval));
				_logger.Information($"PLC Data Fetcher sampling interval set to {interval} seconds.");
			}
			else
			{
				_timer = new PeriodicTimer(TimeSpan.FromSeconds(30)); // Giá trị mặc định nếu không đọc được
				_logger.Information("Could not read sampling interval from configuration. Using default 30 seconds.");
			}
		}
		...
		protected override async Task ExecuteAsync(CancellationToken stoppingToken)
		{
			...
			while (!stoppingToken.IsCancellationRequested)
			{
				...
				{
					_logger.Error($"Worker failed: {ex.Message}. Restarting in 5s...");
		...
		private async Task RunWorkerLoop(CancellationToken stoppingToken)
		{
			...
			_logger.Information("PLC Read Data is starting.");
			// Tạo kết nối đến PLC
			...
			while (!stoppingToken.IsCancellationRequested)
			{
				...
				using (var scope = _serviceProvider.CreateScope())
				{
					...
					isConnected = await _plcService.ConnectPLCAsync();
					if (!isConnected)
					{
						_logger.Error("Delay 10s trước khi ket noi lai.");
					...
					else
					{
						while (!stoppingToken.IsCancellationRequested)
						{
							...
							_logger.Information($"Lay Du Lieu Thanh Cong. Delay: {_timer.Period} ");
						}
					}
				}
			}
